...
class Ambulans {
    ...
    static async getById(id) {
        try {
            const [rows] = await db.query('SELECT * FROM ambulans WHERE id = ? AND active = true', [id]);
            return rows[0];
        } catch (error) {
    ...
    static async update(id, ambulansData) {
        try {
            const [result] = await db.query('UPDATE ambulans SET ? WHERE id = ?', [ambulansData, id]);
    ...
    static async delete(id) {
        try {
            // Soft delete dengan mengubah status active
            const [result] = await db.query('UPDATE ambulans SET active = false WHERE id = ?', [id]);
            return result.affectedRows > 0;
        } catch (error) {
    ...
    static async getByProvider(providerId) {
        try {
            const [rows] = await db.query('SELECT * FROM ambulans WHERE provider_id = ? AND active = true', [providerId]);
            return rows;
        } catch (error) {
    ...
    static async verify(id) {
        try {
            const [result] = await db.query('UPDATE ambulans SET verified = true WHERE id = ?', [id]);
            return result.affectedRows > 0;
        } catch (error) {
